...
static inline void __nodes_complement(nodemask_t *dstp, const nodemask_t *srcp,
				      unsigned int nbits)
{
...
static inline void __nodes_shift_right(nodemask_t *dstp, const nodemask_t *srcp,
				       int n, int nbits)
{
...
static inline void __nodes_shift_left(nodemask_t *dstp, const nodemask_t *srcp,
				      int n, int nbits)
{
...
#define next_weighted_interleave_node(n, src, mem_weights, mem_current)        \
	__next_weighted_interleave_node((n), &(src), (short *)&(mem_weights),  \
					(short *)&(mem_current))
static inline int __next_weighted_interleave_node(int n, const nodemask_t *srcp,
						  short *mem_weights,
						  short *mem_current)
{
	int candidate;
	if (mem_current[n] < mem_weights[n]) {
		mem_current[n] += 1;
		return n;
	} else {
		mem_current[n] = 0;
		candidate =
			min_t(int, MAX_NUMNODES,
			      find_next_bit(srcp->bits, MAX_NUMNODES, n + 1));
		mem_current[candidate] += 1;
		return candidate;
	}
}
...
static inline int __node_remap(int oldbit, const nodemask_t *oldp,
...
static inline void __nodes_remap(nodemask_t *dstp, const nodemask_t *srcp,
				 const nodemask_t *oldp, const nodemask_t *newp,
				 int nbits)
{
...
#define NODEMASK_SCRATCH(x)                                                    \
	NODEMASK_ALLOC(struct nodemask_scratch, x, GFP_KERNEL | __GFP_NORETRY)
